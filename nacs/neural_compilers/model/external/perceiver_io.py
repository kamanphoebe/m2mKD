...
from neural_compilers.model.nc import tokenizers as tok
...
class PerceiverIO(nn.Module):
    def __init__(
        ...
        *,
        ...
        from_bchw=False,
        use_tokenizer=False,
        tokenizer_name="ConvNeXtImageTokenizer",
        tokenizer_kwargs=None,
    ):
        super().__init__()
        self.as_classifier = as_classifier
        self.from_bchw = from_bchw
        if use_tokenizer:
            assert not from_bchw
            tokenizer_cls = getattr(tok, tokenizer_name)
            tokenizer_kwargs = keep_valid_backups(
                tokenizer_cls, tokenizer_kwargs, input_dim=dim, repr_dim=latent_dim
            )
            self.tokenizer = tokenizer_cls(**(tokenizer_kwargs or {}))
            dim = self.tokenizer.output_dim
        else:
            self.tokenizer = None
            dim = latent_dim
        ...
        if as_classifier:
            self.classification_query = torch.nn.Parameter(torch.randn(1, queries_dim))
    ...
    def forward(self, data, mask=None, queries=None):
        ...
        # if queries is not provided and we doin classification,
        # set the query
        if queries is None and self.as_classifier:
            queries = self.classification_query
        ...
        if self.as_classifier:
            # Squeeze out the extra set index and return
            return logits[:, 0, :]
        else:
            return logits
...
class PerceiverLM(nn.Module):
    def __init__(self, *, dim, num_tokens, max_seq_len, **kwargs):
        ...
        self.perceiver_io = PerceiverIO(
            dim=dim, queries_dim=dim, logits_dim=num_tokens, **kwargs
        )
