...
from .vision_transformer import Mlp, Attention, PatchEmbed, HybridEmbed
...
class VMoE(Backbone):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=3,
                 ...
                 pre_stitch_pos=None, post_stitch_pos=None, stitch_dim=None, out_feature='last_feat'):
        super().__init__()
    ...
    def forward(self, x):
        ...
        #     x = self.head(x)
        # return x
        # x = x[:, 1:, :].view(x.size(0), 14, 14, x.size(2))
        x = x.view(x.size(0), 14, 14, x.size(2))
