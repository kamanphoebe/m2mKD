...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, start=None, end=None):
        ...
        blocklst = self.blocks if start is None else self.blocks[start:end]
        for blk in blocklst:
            x = blk(x)

        if not end or end == len(self.blocks):
            x = self.norm(x)
            return x[:, 0]
    ...
    def forward(self, x, start=None, end=None):
        ...
        x = self.forward_features(x, start, end)
        if end is None or end == len(self.blocks):
            x = self.head(x)
        return x
...
def vit_huge_patch14_224(pretrained=False, **kwargs):
    model = VisionTransformer(patch_size=14, embed_dim=1280, depth=32, num_heads=16, mlp_ratio=4, **kwargs)
    model.default_cfg = default_cfgs['vit_huge_patch14_224']
    return model


@register_model
def vit_huge_patch14_392(pretrained=False, **kwargs):
    model = VisionTransformer(img_size=392, patch_size=14, embed_dim=1280, depth=32, num_heads=16, mlp_ratio=4, **kwargs)
    model.default_cfg = _cfg()
    return model
